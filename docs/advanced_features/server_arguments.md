...
| `--nsa-topk-backend` | Choose the NSA indexer top-k backend for unfused topk selection. `torch` and `flashinfer` backends currently require `SGLANG_NSA_FUSE_TOPK=false`. | `sgl-kernel` | `sgl-kernel`, `torch`, `flashinfer` |
