...
| `--nsa-topk-backend` | Choose the NSA indexer top-k backend. The `torch` backend currently requires `SGLANG_NSA_FUSE_TOPK=false`. | `sgl_kernel` | `sgl_kernel`, `torch`, `flashinfer` |
