...
| `SGLANG_NSA_TOPK_FLASHINFER_DETERMINISTIC` | Use deterministic FlashInfer topk kernels when `--nsa-topk-backend=flashinfer` | `false` |
| `SGLANG_NSA_TOPK_FLASHINFER_TIE_BREAK` | Tie-break mode for FlashInfer NSA topk when `--nsa-topk-backend=flashinfer`. Valid values are `0`, `1`, and `2`; non-zero values imply deterministic topk in FlashInfer. | `0` |
